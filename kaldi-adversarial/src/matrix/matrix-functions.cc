...
template<typename Real> void ComputeFftMatrix(Matrix<Real> *M) {
  //KALDI_ASSERT(M->NumRows() == M->NumCols());
  MatrixIndexT K = M->NumRows();
  MatrixIndexT N = M->NumCols();
  ...
  for (MatrixIndexT k = 0; k < K; k++)
    for (MatrixIndexT n = 0; n < N/2; n++) { // /2
      (*M)(k, n) = std::cos( static_cast<double>(-M_2PI)/N * n * k );
      (*M)(k, n+N/2) = std::sin( static_cast<double>(-M_2PI)/N * n * k );
    }
}


template void ComputeFftMatrix(Matrix<float> *M);
template void ComputeFftMatrix(Matrix<double> *M);
