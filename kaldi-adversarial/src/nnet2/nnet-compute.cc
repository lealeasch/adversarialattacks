...
class NnetComputer {
 public:
  ...
  CuMatrixBase<BaseFloat> &GetPreOutput() { return forward_data_[nnet_.NumComponents()-1]; }
...
void NnetComputation(const Nnet &nnet,
                     ...
                     bool pad_input,
                     CuMatrixBase<BaseFloat> *output,
                     CuMatrixBase<BaseFloat> *pre_output) {
  NnetComputer nnet_computer(nnet, input, pad_input, NULL);
  nnet_computer.Propagate();
  output->CopyFromMat(nnet_computer.GetOutput());
  pre_output->CopyFromMat(nnet_computer.GetPreOutput());
}
