...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Create TIME feature files.\n"
        "Usage:  compute-time-feats [options...] <wav-rspecifier> <feats-wspecifier>\n";

    // construct all the global objects
    ParseOptions po(usage);
    TimeOptions time_opts;
    ...
    // Register the TIME option struct
    time_opts.Register(&po);
    ...
    for (; !reader.Done(); reader.Next()) {
      ...
      if (output_format == "kaldi") {
        ...
        HtkHeader header = {
          ...
          static_cast<uint16>( 006 | // TIME
          (time_opts.use_energy ? 0100 : 020000)) // energy; otherwise c0
        };
        p.second = header;
