...
#ifndef KALDI_FEAT_FEATURE_TIME_H_
#define KALDI_FEAT_FEATURE_TIME_H_
...
/// TimeOptions contains basic options for computing TIME features.
struct TimeOptions {
  ...
  TimeOptions() : mel_opts(23),
                  // defaults the #mel-banks to 23 for the TIME computations.
  ...
  void Register(OptionsItf *opts) {
    ...
    opts->Register("num-ceps", &num_ceps,
                   "Number of cepstra in TIME computation (including C0)");
    opts->Register("use-energy", &use_energy,
                   "Use energy (not C0) in TIME computation");
    opts->Register("energy-floor", &energy_floor,
                   "Floor on energy (absolute, not relative) in TIME computation");
    opts->Register("raw-energy", &raw_energy,
                   "If true, compute energy before preemphasis and windowing");
    opts->Register("cepstral-lifter", &cepstral_lifter,
                   "Constant that controls scaling of TIMEs");
...
// This is the new-style interface to the TIME computation.
class TimeComputer {
 public:
  typedef TimeOptions Options;
  explicit TimeComputer(const TimeOptions &opts);
  TimeComputer(const TimeComputer &other);
 ...
 private:
  // disallow assignment.
  TimeComputer &operator = (const TimeComputer &in);
...
typedef OfflineFeatureTpl<TimeComputer> Time;
...
#endif  // KALDI_FEAT_FEATURE_TIME_H_
