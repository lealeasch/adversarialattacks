...
void AlignUtteranceWrapperForced(
    const AlignConfig &config,
    const std::string &utt,
    BaseFloat acoustic_scale,  // affects scores written to scores_writer, if
                               // present
  ...
  if (alignment_writer != NULL && alignment_writer->IsOpen()) {
    std::vector<int32> pdfalignment;
    pdfalignment.resize(alignment.size());
    for(int32 i = 0; i < alignment.size(); i++)
        pdfalignment[i] = trans_model.TransitionIdToPdf(alignment[i]);
    alignment_writer->Write(utt, pdfalignment);
  }
